...
function reduce<T>(f: (x:T, y: T)=>(T|T[]), acc: T, l: ConsList<T>): T{
    return !l ? acc : reduce(f, f(acc, head(l)), rest(l))
}

function reduceRight<T>(f: (x:T, y: T)=>T, acc: T, l: ConsList<T>): T {
...
class List<T> {
    ...
    map(f: (_:T)=>T): ConsList<T> {
        function map_aux<U,W>(f: (_:U)=>W, l: ConsList<U>): ConsList<W> {
            return !l ? null : cons(f(head(l)), map_aux(f, rest(l)))
        }
        return map_aux(f, this.head)
    }

    reduce(f: (_:T)=>T, acc: T): T {
        function reduce_aux(f: (x:T, y: T)=>(T), acc: T, l: ConsList<T>): T{
            return !l ? acc : reduce(f, f(acc, head(l)), rest(l))
        }
        return reduce_aux(f, acc, this.head)
    }

    filter(f: (_:T)=>boolean) {
        function filter_aux(f: (_:T)=>boolean,l:ConsList<T>):ConsList<T>{
            return !l ? null :  f(head(l)) ? cons(head(l), filter(f, rest(l))) : filter(f, rest(l))
        }
        return filter_aux(f, this.head)
    }
    
    concat<T>(l1:ConsList<T>, l2:ConsList<T>): ConsList<T> {
        return l1 ? cons(head(l1), concat(rest(l1), l2)) : l2
}
